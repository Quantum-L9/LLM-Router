...
import { Provider, MessageStrategy, SonarModel, } from '../types.js';
// ═══════════════════════════════════════════════════════════════
// CLIENT
// ═══════════════════════════════════════════════════════════════
export class PerplexityClient {
     ...
     */
    async complete(config, systemPrompt, userPrompt, assistantContext) {
        ...
        if (config.recencyFilter !== 'none') {
            requestBody.web_search_options.search_recency_filter =
                config.recencyFilter;
        }
        // Add domain filter
        if (config.domainFilter.length > 0) {
            requestBody.web_search_options.search_domain_filter =
                config.domainFilter;
        }
        ...
        try {
            ...
            throw new PerplexityError(`Perplexity API call failed: ${error instanceof Error ? error.message : String(error)}`, config);
        }
    }
    /**
     ...
     */
    async completeWithConsensus(config, systemPrompt, userPrompt, assistantContext) {
        ...
        const promises = Array.from({ length: config.variations }, () => this.complete(config, systemPrompt, userPrompt, assistantContext));
    ...
    calculateActualCost(config, usage) {
        if (!usage)
            return config.estimatedCostPerCall;
        const COST_PER_1K = {
