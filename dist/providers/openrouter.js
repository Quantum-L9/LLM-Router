...
import { GeneralModel, Provider, } from '../types.js';
// ═══════════════════════════════════════════════════════════════
...
export class OpenRouterClient {
    client;
    appName;
    constructor(apiKey, appName = 'L9-SEO-Bot') {
     ...
     */
    async complete(config, systemPrompt, userPrompt) {
        ...
        try {
            ...
            throw new OpenRouterError(`OpenRouter API call failed [${modelId}]: ${error instanceof Error ? error.message : String(error)}`, config);
        }
    }
    /**
     * Execute a vision completion with image(s).
     */
    async completeWithVision(config, systemPrompt, userPrompt, imageUrls) {
        const startTime = Date.now();
        const modelId = OPENROUTER_MODEL_IDS[config.model];
        ...
        try {
            ...
            throw new OpenRouterError(`OpenRouter Vision call failed [${modelId}]: ${error instanceof Error ? error.message : String(error)}`, config);
        }
    }
    /**
     * Execute with automatic fallback chain.
     */
    async completeWithFallback(config, fallbackModels, systemPrompt, userPrompt) {
        ...
        try {
            ...
            throw new OpenRouterError(`All models failed (primary: ${config.model}, fallbacks: ${fallbackModels.join(', ')})`, config);
        }
    }
