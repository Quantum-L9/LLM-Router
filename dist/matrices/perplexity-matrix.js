 ...
 */
import { SonarModel, SearchContextSize, SearchMode, RecencyFilter, MessageStrategy, TaskType, TaskComplexity, } from '../types.js';
// ═══════════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════════
function resolveContextSize(task) {
    ...
    if (task.type === TaskType.COMPETITOR_RESEARCH ||
    ...
    if (task.type === TaskType.CITATION_CHECK)
        return SearchContextSize.LOW;
    // Link prospecting benefits from breadth
    if (task.type === TaskType.LINK_PROSPECTING)
        return SearchContextSize.HIGH;
...
function resolveMaxTokens(task) {
    if (task.expectedOutputTokens)
        return task.expectedOutputTokens;
    // Deep research gets generous allocation
    if (task.complexity === TaskComplexity.CRITICAL)
        return 4096;
    // Strategic reasoning needs room to think
    if (task.type === TaskType.STRATEGIC_REASONING)
        return 3072;
...
// ═══════════════════════════════════════════════════════════════
function resolveMessageStrategy(task) {
...
// COST ESTIMATOR
// ═══════════════════════════════════════════════════════════════
export function estimatePerplexityCost(config) {
...
function buildResolutionReason(task, model) {
