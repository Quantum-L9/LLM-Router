...
try {
  ...
  const unexpected = artifact.files.map(entry => entry.path).filter(path => !(
    path === 'package.json' || path === 'README.md' || path === 'ARCHITECTURE.md' || path === 'RUNBOOK.md' || path.startsWith('dist/') || path.startsWith('packages/graphiti-memory-client/')
  ));
  ...
  // Public npm only. Nested @quantum-l9/graphiti-memory-client is file: inside the pack.
  // Do not rewrite the org scope to GitHub Packages and do not require NODE_AUTH_TOKEN.
  await writeFile(join(workspace, '.npmrc'), 'registry=https://registry.npmjs.org\n');
  const smokeEnv = { ...process.env };
  delete smokeEnv.NODE_AUTH_TOKEN;
  delete smokeEnv.npm_config__authToken;
  runNpm(['install', '--ignore-scripts', '--no-audit', '--no-fund', '--package-lock=false', tarball], { cwd: workspace, stdio: 'inherit', env: smokeEnv });
  const smoke = `
